...
/**
 * Canvas grid that lights up around the pointer. Ported from React Bits
 * (JSX + a stylesheet) to TSX, with the two CSS rules moved into globals.css —
 * this project has no component stylesheets.
 *
 * The rAF loop stops itself once no cell is lit, so an idle hero costs nothing.
 * Don't "optimise" that into a permanent loop.
 */
...
export function CursorGrid({
  ...
  // Synced in an effect, not during render — the draw loop reads this ref, and
  // writing to a ref while rendering is a React 19 violation.
  useEffect(() => {
    propsRef.current = {
      cellSize,
      color,
  ...
  useEffect(() => {
    ...
    // No pointer to follow, or the visitor asked for less motion: render
    // nothing and attach nothing.
    if (
